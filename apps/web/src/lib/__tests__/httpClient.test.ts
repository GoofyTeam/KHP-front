...
describe('httpClient (browser environment)', () => {
  ...
  it('performs GET requests without requesting CSRF tokens', async () => {
    ...
    const getCsrfSpy = vi.spyOn(httpClient as { getCsrfToken: () => Promise<string | null> }, 'getCsrfToken')
  ...
  it('attaches CSRF token and keeps FormData headers untouched on POST', async () => {
    ...
    expect((config.headers as Record<string, string>)['X-XSRF-TOKEN']).toBe('token123')
  ...
  it('unwraps validation errors from 422 responses', async () => {
    ...
    fetchMock.mockResolvedValue({
      ...
      headers: {
        get: (key: string) =>
          key.toLowerCase() === 'content-type' ? 'application/json' : key.toLowerCase() === 'content-length' ? '1' : null,
      },
    ...
    await expect(httpClient.post('/users', { email: 'bad' })).rejects.toThrow('Adresse invalide')
  ...
  it('returns an empty object when response is not JSON', async () => {
    fetchMock.mockResolvedValue({
      ...
      headers: {
        get: (key: string) =>
          key.toLowerCase() === 'content-type' ? 'text/plain' : key.toLowerCase() === 'content-length' ? '0' : null,
      },
      text: () => Promise.resolve(''),
    })
  ...
  it('throws a friendly message for unauthorized responses', async () => {
    fetchMock.mockResolvedValue({
      ...
      headers: {
        get: (key: string) =>
          key.toLowerCase() === 'content-type' ? 'application/json' : key.toLowerCase() === 'content-length' ? '0' : null,
      },
      text: () => Promise.resolve(''),
    })
