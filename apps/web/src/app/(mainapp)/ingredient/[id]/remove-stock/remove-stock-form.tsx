...
import { removeIngredientQuantityAction } from "@/app/(mainapp)/ingredient/actions";
import {
  Form,
  FormControl,
  FormField,
...
export function RemoveStockForm({ ingredient }: RemoveStockFormProps) {
  ...
  const handleSubmit = async (data: RemoveStockFormData) => {
    ...
    try {
      ...
      if (quantity > maxQuantity) {
        setApiError(`Quantity cannot exceed ${maxQuantity} ${ingredient.unit}`);
        return;
      }

      const result = await removeIngredientQuantityAction(ingredient.id, {
    ...
    return (
      <div className="text-center space-y-4">
        ...
        <p className="text-khp-text-secondary">
          There is no stock available to remove for this ingredient.
        </p>
      </div>
    );
  }
        ...
        <div className="mt-4 p-3 bg-khp-background-secondary rounded-lg border border-khp-border">
          ...
          <div className="flex justify-between items-center mt-1">
            ...
            <span className="font-medium text-khp-warning">
              -{removeQuantity.toFixed(3)} {ingredient.unit}
            </span>
          </div>
