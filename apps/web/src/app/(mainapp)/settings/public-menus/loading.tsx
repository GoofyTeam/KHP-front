export default function Loading() {
  return (
    <div className="h-full flex items-center justify-center lg:p-4 w-full">
      <div className="w-full">
        <div className="lg:p-8">
          <div className="max-w-2xl mx-auto space-y-8 w-full">
            <div className="bg-khp-surface rounded-2xl shadow-lg border border-khp-primary/20 overflow-hidden w-full">
              <div className="bg-gradient-to-r from-khp-primary/5 to-khp-primary/10 px-6 py-5 border-b border-khp-primary/20">
                <div className="animate-pulse space-y-2">
                  <div className="h-6 bg-khp-primary/20 rounded w-1/2"></div>
                  <div className="h-4 bg-khp-primary/10 rounded w-3/4"></div>
                </div>
              </div>
              <div className="p-6 space-y-6">
                <div className="animate-pulse space-y-4">
                  <div className="flex items-center justify-between">
                    <div className="space-y-2">
                      <div className="h-4 bg-khp-primary/10 rounded w-1/3"></div>
                      <div className="h-3 bg-khp-primary/10 rounded w-2/3"></div>
                    </div>
                    <div className="h-6 w-11 bg-khp-primary/20 rounded-full"></div>
                  </div>

                  <div className="space-y-2">
                    <div className="h-4 bg-khp-primary/10 rounded w-1/4"></div>
                    <div className="h-12 bg-khp-primary/10 rounded"></div>
                  </div>

                  <div className="pt-4">
                    <div className="h-12 bg-khp-primary/20 rounded"></div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}
