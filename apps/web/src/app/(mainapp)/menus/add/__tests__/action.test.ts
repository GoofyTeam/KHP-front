...
import {
  createMenuAction,
  updateMenuAction,
} from "@/app/(mainapp)/menus/add/action";
...
describe("createMenuAction", () => {
  test("serialises all menu fields into FormData", async () => {
    ...
    const [endpoint, formData] = httpClientMock.post.mock.calls[0] as [
      string,
      FormData,
    ];
    ...
    expect(formData.get("items[0][entity_id]")).toBe(
      payload.items[0].entity_id,
    );
    expect(formData.get("items[0][entity_type]")).toBe(
      payload.items[0].entity_type,
    );
    expect(formData.get("items[0][quantity]")).toBe(
      String(payload.items[0].quantity),
    );
    expect(formData.get("items[0][unit]")).toBe(payload.items[0].unit);
    expect(formData.get("items[0][location_id]")).toBe(
...
describe("updateMenuAction", () => {
  test("sends PUT FormData payload with updated fields", async () => {
    ...
    const newImage = new File(["updated"], "updated.png", {
      type: "image/png",
    });
  ...
  test("omits image field when it is an existing URL string", async () => {
    ...
    const [, formData] = httpClientMock.post.mock.calls[0] as [
      string,
      FormData,
    ];
    expect(formData.get("_method")).toBe("PUT");
