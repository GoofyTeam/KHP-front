...
import { addPreparationQuantityAction } from "../actions";
import {
  Form,
  FormControl,
  FormField,
...
export function AddStockForm({ preparation }: AddStockFormProps) {
  ...
  const existingLocations = preparation.quantities.map((q) => ({
    quantity: q.quantity,
    location: q.location,
  }));
  ...
  const handleSubmit = async (data: AddStockFormData) => {
    ...
    try {
      ...
      const result = await addPreparationQuantityAction(preparation.id, {
        ...
        <div className="mt-4 p-3 bg-khp-background-secondary rounded-lg border border-khp-border">
          ...
          <div className="flex justify-between items-center mt-1">
            ...
            <span className="font-medium text-green-600">
              +{addQuantity.toFixed(3)} {preparation.unit}
            </span>
          </div>
