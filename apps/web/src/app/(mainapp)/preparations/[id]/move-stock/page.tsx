...
interface MoveStockPageProps {
  params: Promise<{
    id: string;
  }>;
}

export default async function MoveStockPage({ params }: MoveStockPageProps) {
  const { id } = await params;

  const [preparationResult, locations] = await Promise.all([
    query({
  ...
  if (preparationResult.error) {
    console.error("GraphQL error:", preparationResult.error);
    throw preparationResult.error;
  }

  const preparation = preparationResult.data.preparation;
      ...
      <div className="flex flex-col gap-8 justify-center items-center w-full lg:w-1/2">
        <div className="text-center space-y-4 w-full max-w-lg relative">
          <div className="flex justify-center items-center gap-2">
            <ArrowLeftRight className="h-8 w-8 text-khp-primary" />
